...
namespace Recommenders.RWRBased {
    ...
    public enum EdgeType { UNDEFINED, LIKE, FRIENDSHIP, FOLLOWSHIP, COMMUNICATION, ATHORSHIP, PURCHASE, ETC }
    ...
    public class Recommender {
        ...
        public List<KeyValuePair<long, double>> Recommendation(int indTargetUser, float dampingFactor, int nIteration) {
            Model model = new Model(graph, dampingFactor, indTargetUser);
            model.run(nIteration);

            // Make an exception list of items for target user
            var linksOfTargetUser = new List<int>();
            foreach (ForwardLink link in graph.edges[indTargetUser])
        ...
        public List<KeyValuePair<long, double>> Recommendation(int indTargetUser, float dampingFactor, int nIteration, int topN) {
            var recommendation = Recommendation(indTargetUser, dampingFactor, nIteration);
